...
namespace CChess
{
    ...
    internal class Database
    {
        ...
        public static string InitEccoData()
        {
            ...
            static void SetEccoRecordRegstr(SortedDictionary<string, string[]> records)
            {
                char UnorderChar = '|', OrderChar = '*', SeparateChar = '/', OroneChar = '?', ExceptChar = '除';

                // 删除此前已加入的相同着法
                //void DeleteSameMvstr(SortedDictionary<char, List<string>> boutStrs, int color, string qstr)
                //{
                ...
                void SetBoutStrs(SortedDictionary<char, List<string>> boutStrs, string sn, string mvstrs, bool isPreMvstrs)
                {
                    ...
                    string mv = string.Format(@$"[{Piece.PGNZHChars()}]{4}"),
                        moveStr = string.Format(@$"(?:{mv}(?:／{mv})*)"),
                        premStr = string.Format(@$"(?:{mv}(?:[、／和，黑可走]{1,4}{mv})*)"),
                        rich_mvStr = string.Format(@$"({moveStr}|……)(\s?\(不.先后[)，])?(?:[^\da-z]*?此前.*?走((?:过除)?{premStr}).*?\))?");

                    // 捕捉一个回合着法：1.序号，2-4.首着、“不分先后”、“此前...”，5-7.着法、“不分先后”、“此前...”
                    var matches = Regex.Matches(mvstrs, string.Format(@$"([\da-z]).\s?{rich_mvStr}(?:，{rich_mvStr})?"));
                ...
                string GetRowcol(string zhStr, bool isGo, Manual manual)
                {
                    ...
                    return success ? manual.ManualMove.CurMove.CoordPair.RowCol : "";
                }
                ...
                List<string> GetRowcolList(string mvstr, bool isOrder, Manual manual)
                {
                    ...
                    foreach(Match match in matches.Cast<Match>())
                    {
                        ...
                        string rowcol = GetRowcol(zhStr, manual, isGo);
                        if(rowcol.Length > 0)
                    ...
                    for(int i = 0;i < count;++i)
                    {
                        ...
                        // 着法数量大于1，或属于多顺序着法的非最后着法
                            ...
                            rowcols = (mvstr.Contains(ExceptChar)
                                    ? string.Format($"{moveRegStr}*?")
                                    : string.Format($"(?:{rowcols}){indexStr}{num}"));
                ...
                string GetRegstr(SortedDictionary<char, List<string>> boutStrs, string sn, Manual manual)
                {
                    ...
                    return string.Format($"~{regStr[0]}") + moveRegStr + string.Format($"*?-{regStr[1]}");
                }
                ...
                foreach(var record in records)
                {
                    ...
                    if(pre_sn.Length > 0)
                    {
                        ...
                        if(match.Success)
                            ...
                            boutStrs.Add('1', new List<string> { match.Groups[1].Value, match.Groups[2].Value });
                        else
            ...
            var records = GetEccoRecords(DownHtmls());
            SetEccoRecordRegstr(records);

            string result = "";
            foreach(var record in records)
                result += string.Join(", ", record.Value) + "\n";
